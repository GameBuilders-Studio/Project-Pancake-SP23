...
public class ExtendedScriptableObjectDrawer : PropertyDrawer 
{
		...
		if (property.isExpanded) 
		{
			...
			if (prop.NextVisible(true)) 
			{
				do 
				{
					...
					float height = 
						EditorGUI.GetPropertyHeight(subProp, null, true) + EditorGUIUtility.standardVerticalSpacing;
					totalHeight += height;
				}
	...
	public override void OnGUI (Rect position, SerializedProperty property, GUIContent label)
	{
		...
		if (
			...
			&& property.serializedObject.targetObject is ScriptableObject @object
		) 
		{
			propertySO = @object;
		}
	...
	static ScriptableObject CreateAssetWithSavePrompt (Type type, string path)
	{
		path = EditorUtility.SaveFilePanelInProject(
			"Save ScriptableObject", type.Name+".asset", "asset", "Enter a file name for the ScriptableObject.", path);
